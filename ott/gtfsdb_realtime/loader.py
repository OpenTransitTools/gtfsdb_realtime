...
def load_agency_gtfsrt_data(session, agency_id, alerts_url, trips_url, vehicles_url):
    ret_val = True

    if alerts_url:
        r = process(session, agency_id, alerts_url)
        if not r:
            ret_val = False

    if trips_url:
        r = process(session, agency_id, trips_url)
        if not r:
            ret_val = False

    if vehicles_url:
        r = process(session, agency_id, vehicles_url)
        if not r:
            ret_val = False
...
def process(session, agency_id, feed_url, clear_tables_first=True):
    """ will grab a GTFS-RT feed, and store it in a database
    """
...
def main():
    ...
    no_errors = load_agency_gtfsrt_data(session, args.agency, aurl, turl, vurl)
    if no_errors:
